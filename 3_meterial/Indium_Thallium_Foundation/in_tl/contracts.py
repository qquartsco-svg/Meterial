...
@dataclass
class ElectronicsAssessment:
 ito_touch_display: str; cigs_absorber: str; notes: List[str]=field(default_factory=list)
@dataclass
class ToxicityAssessment:
 tl_acute_class: str; indium_exposure_context: str; notes: List[str]=field(default_factory=list)
@dataclass
class ClaimPayload:
 claim_text: str; claimed_in_tl_same_safety_class: bool=False; claimed_infinite_indium_byproduct: bool=False
@dataclass
class ScreeningReport:
 verdict: Verdict; omega: float; flags: List[str]=field(default_factory=list); reasoning: List[str]=field(default_factory=list)
@dataclass
class HealthReport:
 omega_tech: float; omega_safety: float; omega_supply: float; omega_recycling: float; omega_economics: float
...
class FoundationReport:
 electronics: Optional[ElectronicsAssessment]=None; toxicity: Optional[ToxicityAssessment]=None
